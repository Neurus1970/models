...
router.get('/models/scoring/smes/:id', (req, res) => {

  var initialTime = new Date();

  var posicionElemento = config.settings.smesScoreData.findIndex(({id}) => id == req.params.id);
  ...
  if (posicionElemento == -1) {
   ...
   var salida = {
      ...
      hits: 1,
      debtors: config.settings.smesScoreData[posicionElemento]
    }
...
router.get('/models/scoring/smes', (req, res) => {
  ...
  if (req.query.name !== undefined) {
    ...
    config.settings.smesScoreData.forEach(v => {
      if(names.every(name => v.name.includes(name))) deudores.push(v);
    });
  } else
    deudores = config.settings.smesScoreData;
  ...
  if (deudores.length == 0) {
    ...
    if (deudoresAmostrar.length != 0) {
      ...
      nextPage = '/models/scoring/smes?page='.concat(pageNumber+2);
      prevPage = '/models/scoring/smes?page='.concat(pageNumber);
